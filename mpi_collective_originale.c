...
MPI_process_info *mpi_init(int rank) {
  ...
  memset(sock_table, 0xFF, WORD_SIZE * sizeof(int));
...
int mpi_recv(void *buf, int count, MPI_Datatype datatype, int source, int tag) {
  ...
  ssize_t err = recv(MPI_PROCESS->socket_fd[source], tag_recv, sizeof(int), 0);
  if (err == -1) {
    printf("failed recv\n");
    exit(EXIT_SUCCESS);
  }
  ...
  err = recv(MPI_PROCESS->socket_fd[source], buf_recv, size, 0);
  if (err == -1) {
    printf("failed recv\n");
    exit(EXIT_SUCCESS);
  }
