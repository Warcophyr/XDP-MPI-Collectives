...
void insert_lfence() {
  // Il clobber "memory" agisce come barriera per il compilatore
  __asm__ __volatile__("lfence" ::: "memory");
}

void insert_mfence() { __asm__ __volatile__("mfence" ::: "memory"); }
...
int extract_5tuple(int sockfd, struct socket_id *id) {
  ...
  // the cake is a lie
  inet_pton(AF_INET, GRECALE_IP, &id->dst_ip);
...
int create_udp_socket(int port) {
  ...
  // if (setsockopt(socket_fd, SOL_SOCKET, SO_ZEROCOPY, &opt, sizeof(opt)) < 0)
  // {
  //   perror("setsocketopt SO_ZEROCOPY\n");
  // }
  // int busy_us = 10; // microseconds
  // if (setsockopt(socket_fd, SOL_SOCKET, SO_BUSY_POLL, &busy_us,
  // sizeof(busy_us))< 0){
  //   perror("setsocketopt  SO_BUSY_POLL\n");
  // }
  ...
  // addr.sin_addr.s_addr = inet_addr(MAESTRALE_IP);
...
int connect_tcp_peer(int peer_rank, int my_rank) {
  int socket_fd;
  struct sockaddr_in addr;
  char ip[] = "127.0.0.1";
  int port = FALL_BACK_PORT + peer_rank;
  socket_fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
...
MPI_process_info *mpi_init(int rank, int mpi_sockets_map_fd,
  ...
  memset(sock_table_tcp, 0xFF, WORD_SIZE * sizeof(int));

  int server_fd = create_tcp_socket(FALL_BACK_PORT + rank);
  ...
  for (int i = 0; i < WORD_SIZE; ++i) {
    ...
    if (rank < i) {
      sock_table_tcp[i] = connect_tcp_peer(i, rank);
      // printf("fd: %d\n", sock_table_tcp);
    } else if (rank > i) {
      ...
      sock_table_tcp[i] = conn_fd;
    }
  }
  mpi_process_info->socket_tcp_fd = sock_table_tcp;
...
int flush_udp_rx(int fd) {
  char buf[65536]; /* big enough for any UDP datagram */
  for (;;) {
    ssize_t r = recv(fd, buf, sizeof(buf), MSG_DONTWAIT);
    if (r > 0) {
      /* dropped r bytes (one datagram). continue draining */
      continue;
    }
    if (r == -1) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        /* no more datagrams right now — RX queue drained */
        return 0;
      }
      /* real error */
      return -1;
    }
    /* r == 0 : for datagram sockets this is unusual, treat as drained */
    return 0;
  }
}

inline int datatype_size_in_bytes(int count, MPI_Datatype datatype) {
  int res = -1;
  switch (datatype) {
  case MPI_CHAR:
...
inline int bytes_size_in_unit(int count, MPI_Datatype datatype) {
  int res = -1;
  switch (datatype) {
  case MPI_CHAR:
    res = count / sizeof(char);
...
int __mpi_send_tcp(const void *buf, int count, MPI_Datatype datatype, int root,
  ...
  send(MPI_PROCESS->socket_tcp_fd[dest], message, total_size, 0);
  free(message);
  return count;
}
int __mpi_recv_tcp(void *buf, int count, MPI_Datatype datatype, int source,
                   int tag) {
  int size = datatype_size_in_bytes(count, datatype);
  if (size < 1) {
    return -1;
  ...
  while (total < total_size) {
    ...
    ssize_t n = recv(MPI_PROCESS->socket_tcp_fd[source], message + total,
                     total_size - total, 0);
    if (n > 0) {
      total += n;
      // } else if (n == 0) {
...
int __mpi_send_tcp_lost_packet(MPI_Datatype datatype, int root, int dest,
  ...
  send(MPI_PROCESS->socket_tcp_fd[dest], message, MPI_HEADER, 0);
  free(message);
  return 0;
}
...
int __mpi_recv_tcp_lost_packet(MPI_Datatype datatype, int source, int tag) {
  ...
  int err = recv(MPI_PROCESS->socket_tcp_fd[source], message, MPI_HEADER, 0);
  if (err == -1) {
    printf("failed recv\n");
    exit(EXIT_SUCCESS);
  }
...
static int __mpi_send(const void *buf, int count, MPI_Datatype datatype,
  ...
  if (total_size > PAYLOAD_SIZE) {
    ...
    void *id_send = (char *)message + (sizeof(char) * 4) + (sizeof(int) * 3) +
                    sizeof(MPI_Collective) + sizeof(MPI_Datatype) +
                    (sizeof(int) * 2) + sizeof(unsigned long);
    generic_hton(id_send, &MPI_PROCESS->ids[root][dest], sizeof(unsigned long),
                 1);
...
int mpi_recv_no_ack(void *buf, int count, MPI_Datatype datatype, int source,
                    int tag) {
  int size = datatype_size_in_bytes(count, datatype);
  if (size < 1) {
    return -1;
  ...
  if (total_size > 1472) {
    ...
    for (size_t i = 0; i < N; i++) {
      ...
      case MPI_CHAR: {
        ...
        // "
        //        "offset: %d len__: %d\n",
        //        MPI_PROCESS->rank, ((char *)fragment[i].arr), i, buf,
        //        bufptr, fragment[i].len, offset, strlen(fragment[i].arr));
    ...
    if (data_size != size) {
      free(message);
      printf("RANK: %d seq: %lu, recv:%d expected:%d\n", MPI_PROCESS->rank, seq,
             data_size, size);
      // __mpi_send_tcp_lost_packet(MPI_NACK, MPI_PROCESS->rank, source, tag,
      //                            MPI_SEND);
      // int err = __mpi_recv_tcp(buf, count, datatype, source, tag);
...
int mpi_recv(void *buf, int count, MPI_Datatype datatype, int root, int source,
  ...
  if (total_size > 1472) {
    ...
    // fprintf(stderr, "Rank pre recv: %d  %lf sec\n", MPI_PROCESS->rank,
    //         (double)(pre - start));
    // Receive UDP message using the global socket
    ssize_t received = recvfrom(udp_socket_fd, message, total_size, 0,
                                (struct sockaddr *)&sender_addr, &sender_len);
    ...
    // fprintf(stderr, "Rank post recv: %d  %lf sec\n", MPI_PROCESS->rank,
    //         (double)(post - start));

    if (received < 0) {
      free(message);
      printf("lost packet recv: %d\n", (int)received);
      __mpi_send_tcp_lost_packet(MPI_NACK, MPI_PROCESS->rank, source, tag,
                                 MPI_SEND);
      int err = __mpi_recv_tcp(buf, count, datatype, source, tag);
    ...
    // fprintf(stderr, "Rank end: %d  %lf sec\n", MPI_PROCESS->rank,
    //         (double)(end - start));
  }
  /* code */
  return count;
...
int mpi_barrier_ring(void) {
  ...
  int size = WORD_SIZE;
  int dummy = 0;
  int tag = 9999; // Use a unique tag far from your other operations
  ...
  if (rank == 0) {
    ...
    // fprintf(stderr, "%d: Process next %d, prev %d entering barrier\n", rank,
    //         next, prev);
    __mpi_send_tcp(&dummy, 1, MPI_INT, rank, next, tag, MPI_SEND);
    __mpi_recv_tcp(&dummy, 1, MPI_INT, prev, tag);
  } else {
    // fprintf(stderr, "%d: Process: next %d, prev %d entering barrier\n", rank,
    //         next, prev);
  ...
  // fprintf(stderr, "%d: Process next %d, prev %d exiting barrier\n", rank,
  // next,
  //         prev);
  // sleep(1);
...
int mpi_bcast_ring_xdp_eager(void *buf, int count, MPI_Datatype datatype,
                             int root) {
  int rank = MPI_PROCESS->rank;
  int size = WORD_SIZE;
  int tag = 1; // you can choose any tag
  ...
  if (rank != root) {
    // printf("Process %d receiving from %d\n", rank, prev);
    if (mpi_recv_no_ack(buf, count, datatype, prev, tag) < 0) {
      fprintf(stderr, "Process %d: recv from %d failed\n", rank, prev);
      return -1;
    }
  }
...
int mpi_bcast_linear_xdp(void *buf, int count, MPI_Datatype datatype,
                         int root) {
  int rank = MPI_PROCESS->rank;
  int size = WORD_SIZE;
  int tag = 1; // you can choose any tag
  ...
  if (rank == root) {
    // printf("Process %d (root) sending to %d\n", rank, next);
    __mpi_send(buf, count, datatype, root, next, tag, MPI_BCAST_LINEAR);
    // usleep(1000);
    for (size_t i = 0; i < WORD_SIZE; i++) {
      if (MPI_PROCESS->rank == i)
        continue;
...
int mpi_reduce_linear_sum(const double *buf_src, double *buf_dest, int count,
                          int root) {
  int rank = MPI_PROCESS->rank;
  int size = WORD_SIZE;
  if (rank != root) {
    send(MPI_PROCESS->socket_tcp_fd[root], buf_src, count * sizeof(double), 0);
  } else {
    // printf("recv:%lf, sender: %lf\n", buf_dest[0], buf_src[0]);
    for (size_t i = 0; i < size; i++) {
      if (i == root)
        continue;
      double __buf[count];
      recv(MPI_PROCESS->socket_tcp_fd[i], __buf, count * sizeof(double), 0);
      for (size_t j = 0; j < count; j++) {
        buf_dest[j] += __buf[j];
        printf("recv:%lf, local: %lf\n", buf_dest[j], __buf[j]);
      }
    }
  }
}

int mpi_reduce_linear_max(const double *buf_src, double *buf_dest, int count,
                          int root) {
  int rank = MPI_PROCESS->rank;
  int size = WORD_SIZE;
  if (rank != root) {
  ...
  if (rank == root) {
    for (size_t i = 0; i < size - 1; i++) {
      ...
      case MPI_CHAR: {
        ...
        if (__mpi_recv_tcp(buf_local, count, datatype, MPI_ANY_SOURCE, tag) <
            0) {
          fprintf(stderr, "Process %d: recv from %d failed\n", rank, prev);
          return -1;
        }
