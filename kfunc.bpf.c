...
int kfunc(struct xdp_md *ctx) {
  ...
  if (iph->saddr == bpf_htonl(3232261377)) { // src ip 192.168.101.1
    ...
    //     if (mpi_header[0] != 'M' && mpi_header[1] != 'P' && mpi_header[2] !=
    //     'I' &&
    //         mpi_header[3] != '\0') {
    //       return XDP_PASS;
    //     }
    ...
    //     int src;
    //     void *src_payload = (char *)payload + (sizeof(char) * 4) +
    //     sizeof(int);
    ...
    //           int *size = bpf_map_lookup_elem(&num_process,
    //           &key_num_process); if (size) {
    //             // bpf_printk("size: %d", *size);
    //             int next = (int)(((unsigned)(dst_host + 1)) %
    //             ((unsigned)(*size))); tuple_process inter_dest = {0};
